...
def lambda_handler(event, context):
    ...
    try:
        ...
        s3_resource.Bucket('s3-airbnb-landing', Key = f'date={today_date}/Airbnb_{today_date}.csv')
        print(df)
    except exception as e:
        print('Entering Exception')
        print(str(e))
        df = pd.DataFrame(columns = ['bookinId', 'userId', 'propertyId', 'location', 'startDate', 'endDate', 'price'])
